# CH-011 — Gate 9 Chapter QA v2

**Chapter:** That Is Not What I Imagined  
**Result:** **PASS**  
**Canon Status:** **APPROVED**  
**Word Count:** 583  
**Blocking:** 0  
**Major:** 0  
**Minor:** 2

# Review History

The first Gate 9 review found one prose-standard issue:

- ordinary narration still used too many isolated one-line paragraphs.

Correction:
- routine narration was merged into more natural paragraph flow;
- all four approved scene functions, calibration results, jokes, first-sight details, and privacy rules were preserved exactly in meaning.

Gate 9 was rerun after correction.

# Confirmed
- Lily-only close third.
- four scene functions preserved.
- reverse calibration is separate from Max's CH-010 settings.
- first two Lily-side attempts fail.
- third attempt gives Lily a reliable indirect reflected image of Max for several seconds.
- no direct naked-eye sight.
- no contact/crossing/P2/P3.
- first-sight humor is non-cruel.
- physical awareness does not replace already-earned emotional trust.
- visual privacy/consent rules are explicitly agreed.
- M-002 remains PARTIAL.
- no new F-ID or MT reveal.

Perception after chapter:
- **Lily P1 narrow**
- **Max P1 narrow**
- each requires separate receiver-specific calibration
- C0 / X0

Minor:
1. CH-012 must bring focus back to rent/repairs/family pressure rather than immediately escalating P1.
2. P1 windows remain deliberate and consent-based.

**CH-011 GATE 9 — PASS / CANON-APPROVED**

# Next
**CH-012 — Rent, Repairs, and Other Horrors → Gate 8**
